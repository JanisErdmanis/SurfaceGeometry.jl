 ...
 )
{
  ...
  //parameters.m_subdivision_scheme = new ModifiedButterflyScheme();
  //parameters.m_subdivision_scheme = new QuadraticErrorMinScheme();
  parameters.m_subdivision_scheme = new MidpointScheme();
 ...
 )
{
  ...
  // Doing the integration in time
  surface_tracker.set_all_newpositions(inmsh_Nverticies,newpositions);
  surface_tracker.integrate(m_dt,*actual_dt);
...
//  int triangles_out[Ntriangles_out*3];
  ...
  printf("actual_dt=%f Nverticies=%d  Ntriangles=%d   volume=%f \n",*actual_dt,*outmsh_Nverticies,*outmsh_Ntriangles,surface_tracker.get_volume());
}
